...
export const QuestionModal: React.FC<QuestionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--modal-bg)] p-4 md:p-8 backdrop-blur-md">
      <div
        className={`
          w-full h-full max-w-[1920px] mx-auto rounded-xl shadow-2xl overflow-hidden border-2
      ...
      >
        {/* Header Bar */}
        <div className="bg-[#120a1f]/90 text-white p-6 flex justify-between items-center shadow-lg border-b-2 border-[#7e57c2] shrink-0">
        ...
        {/* Content Area */}
        <div className="flex-grow flex flex-col items-center justify-center p-8 text-center overflow-auto relative">
              ...
              <div className="bg-[#5e35b1] border-4 border-[#ffcc00] p-8 md:p-12 rounded-xl shadow-[0_0_50px_rgba(255,215,0,0.5)] transform animate-pop text-center max-w-4xl w-full">
                ...
                {!showAnswer ? (
                  <>
                    ...
                    <h2 className="text-white text-6xl md:text-8xl font-bold header-font mb-12 drop-shadow-lg">{buzzedTeamName}</h2>
                    <button
                      onClick={handleReveal}
                      className="bg-[#ffcc00] hover:bg-yellow-400 text-black py-6 px-12 rounded-xl font-bold text-3xl shadow-xl transition-transform hover:scale-105 border-b-8 border-yellow-700 active:border-b-0 active:translate-y-2"
                    >
                      REVEAL ANSWER <span className="text-sm block font-normal opacity-50 mt-1">(Space)</span>
                    </button>
                  </>
                ) : (
                  <>
                    ...
                    <div className="flex gap-6 justify-center">
                      <button
                        ...
                        className="bg-red-600 hover:bg-red-500 text-white py-6 px-12 rounded-xl font-bold text-2xl shadow-lg transition-transform hover:scale-105 border-b-8 border-red-800 active:border-b-0 active:translate-y-2"
                      >
                        NO (WRONG) <span className="text-sm block font-normal opacity-50 mt-1">(Key: N)</span>
                      </button>
                      <button
                        onClick={() => handleScore(activeBuzzer, 'add')}
                        className="bg-green-600 hover:bg-green-500 text-white py-6 px-12 rounded-xl font-bold text-2xl shadow-lg transition-transform hover:scale-105 border-b-8 border-green-800 active:border-b-0 active:translate-y-2"
                      >
                        YES (CORRECT) <span className="text-sm block font-normal opacity-50 mt-1">(Key: Y)</span>
                      </button>
                    </div>
                  </>
                )}
          ...
          <div className="max-w-6xl w-full flex items-center justify-center min-h-[40vh]">
            <p className="text-white text-4xl md:text-6xl font-bold leading-tight uppercase tracking-wide drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)]">
              {question.question}
            </p>
          </div>

          {showAnswer && !activeBuzzer && (
            <div className="w-full max-w-5xl flex flex-col items-center mt-8 animate-reveal">
              <div className={`w-full p-8 md:p-10 rounded-xl border-l-8 shadow-2xl bg-gradient-to-r from-[#311b92] to-[#4a148c] relative ${getGlowClass(question.value)} mb-6`}>
                <p className="text-[#ffcc00] text-xl font-bold uppercase mb-2 tracking-wider">Correct Answer</p>
                <p className="text-white text-5xl md:text-7xl font-bold header-font drop-shadow-md">
                  {question.answer}
                </p>
              ...
              <div className="w-full bg-black/40 rounded-lg border border-white/10 p-4 animate-fade-in backdrop-blur-sm">
                ...
                <div className="flex flex-wrap justify-center gap-4">
                  {teams.map(team => (
                    <div key={team.id} className="bg-black/40 rounded px-4 py-2 border border-white/10 flex items-center gap-3">
                      <span className="font-bold text-white max-w-[100px] truncate">{team.name}</span>
        ...
        <div className="p-6 md:p-8 border-t-2 border-[#7e57c2] bg-[#120a1f]/95 shrink-0">
          ...
          {!showAnswer && !activeBuzzer ? (
            <div className="flex justify-center">
              <button
                onClick={handleReveal}
                className="bg-[#ffcc00] hover:bg-yellow-400 text-black font-bold py-4 px-16 rounded-xl shadow-lg transform hover:scale-105 transition-all text-2xl uppercase tracking-wider border-b-8 border-yellow-700 active:border-b-0 active:translate-y-2"
              >
            ...
            <div className="w-full">
              ...
              <div className="flex justify-center">
                <button
                  onClick={handleClose}
                  className="bg-blue-700 hover:bg-blue-600 text-white font-bold py-4 px-12 rounded-xl shadow-lg text-xl transition-all transform hover:scale-105 uppercase tracking-wide border-b-8 border-blue-900 active:border-b-0 active:translate-y-2"
                >
