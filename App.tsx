...
function App() {
  ...
  const handleQuestionClick = (catIndex: number, qIndex: number) => {
    ...
    if (!question.isAnswered) {
      ...
      connectionService.sendMessage({ 
        type: 'QUESTION_OPEN', 
        payload: { text: question.question, value: question.value } 
      });
    }
  };
      ...
      <div className="h-[100dvh] w-screen flex flex-col items-center justify-center bg-[#120a1f] text-white overflow-hidden relative">
         ...
         {showMobileWarning && (
            <MobileWarningModal 
              ...
              onContinue={() => { setShowMobileWarning(false); setAppMode('HOST'); setGameState(GameState.START); }}
            />
         )}
         ...
         <div className="relative z-10 w-full max-w-5xl px-6 flex flex-col h-full py-8 md:py-12 overflow-y-auto no-scrollbar">
            ...
            <div className="flex flex-col md:grid md:grid-cols-2 gap-6 md:gap-8 w-full max-w-4xl mx-auto flex-grow justify-center">
                ...
                >
                   ...
                   <div className="relative z-10 p-6 md:p-8 flex flex-col h-full justify-end">
                      <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-blue-600 text-white flex items-center justify-center mb-4 md:mb-6 shadow-lg group-hover:scale-110 transition-transform">
                      ...
                      <h2 className="text-2xl md:text-3xl font-bold header-font mb-2">JOIN GAME</h2>
                      <p className="text-white/70 mb-4 md:mb-6 text-sm md:text-base">Use your phone as a buzzer. Join a room and compete with others.</p>
                      <div className="flex items-center text-blue-400 font-bold uppercase tracking-widest text-xs md:text-sm group-hover:gap-2 transition-all">
                         Launch Controller <span>&rarr;</span>
                      </div>
                   </div>
                </div>
            </div>
